...
from cliplayer import Player,Engine
import database_related

songs = ("/home/gsy/code/python/me.mp3","/home/gsy/code/python/ask.ogg")
class Preferences:
    def __init__(self):
        self.volume = 1.0
    ...
    def load_from_file(self,instance):       
        ...
        if config_file:
            ...
            for line in config_items:
                ...
                elif key == "play_list_file_path":
                    file_paths = value.split(";")
                    for path in file_paths:
                        self.play_list_file_path.append(path)

    def write_to_file(self,instance):
        if self.path_to_config == None:
        ...
        config_data += "play_list_file_path="
        play_list_file_path = []
        instance.play_list_store.foreach(self.save_play_list_store_func,play_list_file_path)
        joined = ""
        for path in play_list_file_path:                
            joined += str(path) + ";"
        config_data += str(joined)
        
        config_file.write(config_data)

    def save_play_list_store_func(self,model,path,iter,play_list_file_path):
        file_path = model.get_value(iter,1)
        play_list_file_path.append(file_path)
...
class gstreamer_core:
    def __init__(self):
        ...
        # self.source = gst.element_factory_make("uridecodebin",'source')
        ...
        # self.apad = self.convert.get_pad('sink')        
        ...
        # self.player.add(self.source,self.volume,self.convert,self.queue,self.rsmpl,self.sink)
        
        self.player.add(self.filesrc,self.queue,self.decode,self.convert,self.rsmpl,self.volume,self.equalizer,self.sink)
        
        # link the element together
        gst.element_link_many(self.filesrc,self.queue,self.decode)
        gst.element_link_many(self.convert,self.rsmpl, self.volume,self.equalizer,self.sink)
        # gst.element_link_many(self.convert,self.sink)
        # message bus
        bus = self.player.get_bus()
        bus.add_signal_watch()
        bus.connect("message",self.on_message)
        # bus.connect("message::tag",self.on_message_tag)
    
    def on_message(self,bus,message):
        t = message.type
        # End of Stream
        if t == gst.MESSAGE_EOS:
            if myGUI.file_cycle_mode:
                self.player.set_state(gst.STATE_NULL)
                myGUI.play()
            elif myGUI.play_list_mode or myGUI.shuffle_mode:
                myGUI.on_play_next(None)                
            else:
                self.player.set_state(gst.STATE_NULL)                
            myGUI.default_gui()
            ...
            myGUI.is_playing = False
            myGUI.is_file_error = True
            self.player.set_state(gst.STATE_NULL)
            myGUI.default_gui()
    ...
    #     if name == 'audio/x-raw-flate' or name == 'audio/x-raw-int':
    ...
    def seek_simple(self,seek_time_secs):
        self.player.seek_simple(gst.FORMAT_TIME,
                                gst.SEEK_FLAG_FLUSH | gst.SEEK_FLAG_KEY_UNIT,
                                seek_time_secs * gst.SECOND)

    def set_volume(self,value):
        self.player.get_by_name("volume").set_property("volume",float(value))
        return True
    ...
    def query_position(self):
        if gst.STATE_PLAYING in self.player.get_state():
            return self.player.query_position(gst.FORMAT_TIME,None)[0] / gst.SECOND

    def query_duration(self):
        if gst.STATE_PLAYING in self.player.get_state():
            return self.player.query_duration(gst.FORMAT_TIME,None)[0] / gst.SECOND
        
    def set_equalizer_band(self, band, value):
        self.equalizer.set_property(band,float(value))
        print "%s gain set to %s" % (band, value)
...
class Douban:
    ...
    def get_channels(self):
        ...
        for channel in parse_data['channels']:
            self.channel = []       
            for key in channel.keys():
                if key == 'name':
                    self.channel.append(channel[key])
                elif key == 'channel_id':
                    self.channel.append(channel[key])
...
# use gstreamer to get the file tags 
# class TagGetter:
#     def __init__(self):
#         # a list for tag infomations
#         self.taginfo = {}
#         self.tag_player = gst.element_factory_make("playbin","tag_player")
#         self.tag_bus = self.tag_player.get_bus()
#         self.tag_bus.enable_sync_message_emission()
#         self.tag_bus.add_signal_watch()
#         self.tag_bus.connect("message::tag",self.on_message_tag)
...
#             if key == 'title':
#                 self.taginfo['title'] = taglist[key]
#             elif key == 'artist':
#                 self.taginfo['artist'] = taglist[key]
#             elif key == 'album':
#                 self.taginfo['album'] = taglist[key]            
#             elif key == 'track-number':
#                 self.taginfo['track-number'] = taglist[key]
#         if self.taginfo['artist'] and self.taginfo['title']:
#             for key in self.taginfo:
#                 print '%s = %s' % (key,self.taginfo[key])
                
#             self.mainloop.quit()
            
#     def set_file(self,filepath):
#         # set the uri of the playbin to our audio file
#         self.tag_player.set_property('uri','file://' + filepath)
#         self.tag_player.set_state(gst.STATE_PAUSED)
        
#     def run(self):
#         # start the main loop
#         self.mainloop.run()
#         if self.taginfo['artist']:
#             print self.taginfo
#             self.mainloop.quit()                    
...
if __name__ == "__main__":    
    ...
    # myTag = TagGetter()
    DoubanFM = Douban()
    myPlayer = gstreamer_core()
    parser = database_related.FileTagsParser()
    # # test player
    # myPlayer = Player()
    # myEngine = Engine(myPlayer)
